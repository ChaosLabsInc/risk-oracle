...
abstract contract Properties is Asserts, BeforeAfter, PropertiesSpecifications {
    function invariant_no_duplicated_update_types() public returns (bool) {
        ...
        for (uint256 i = 0; i < allUpdateTypes.length; i++) {
            for (uint256 j = i + 1; j < allUpdateTypes.length; j++) {
                t(
                    !_updateTypesEq(allUpdateTypes[i], allUpdateTypes[j]),
                    UPDATE_TYPES_01
                );
            }
        }
    ...
    function invariant_valid_update_types_mapping_mirrors_all_update_types_array()
        public
        returns (bool)
    {
        string[] memory allUpdateTypes = riskOracle.getAllUpdateTypes();
        eq(
            allUpdateTypes.length,
    ...
    function invariant_update_counter_should_equal_number_of_updates()
        public
        returns (bool)
    {
    ...
    function invariant_update_counter_is_monotonically_increasing()
        public
        returns (bool)
    {
