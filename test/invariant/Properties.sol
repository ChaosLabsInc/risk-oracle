...
abstract contract Properties is Asserts, BeforeAfter, PropertiesSpecifications {
    ...
    function invariant_update_history_contains_all_updates() public returns (bool) {
        RiskOracle.RiskParameterUpdate[] memory updateHistory = riskOracle.exposed_updateHistory();

        for (uint256 i = 0; i < riskOracle.updateCounter(); i++) {
            t(_updatesEq(updateHistory[i], riskOracle.getUpdateById(i + 1)), UPDATE_HISTORY_01);
        }

        return true;
    }

    function invariant_update_counter_should_equal_update_history_length() public returns (bool) {
        uint256 updateCounter = riskOracle.updateCounter();
        RiskOracle.RiskParameterUpdate[] memory updateHistory = riskOracle.exposed_updateHistory();
        eq(updateCounter, updateHistory.length, UPDATE_COUNTER_01);
